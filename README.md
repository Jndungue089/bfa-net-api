...
- Migrações em `Infrastructure/Persistence/Migrations` (`InitialCreate`, `AddMobileFeatures`); aplicadas no arranque **apenas em Development**.
...
| Contas | `GET /accounts` · `GET · PATCH /accounts/{id}` · `GET /accounts/{id}/statement?from&to&direction&cursor&limit` |
...
| `Security:AllowedOrigins` | origens do browser para o CSRF |
| `Banking:*` · `RateLimits:*` · `TrustedProxies` | ver `appsettings.json` |
| `Seed:Demo` · `Seed:DemoPassword` · `Seed:DemoPin` | só Development |
